...
-- levelling\". Each level is 'sizeRatio' times bigger than the previous level.
--
...
mergePolicyForLevel conf = go (mergePolicy conf)
  where
  ...
  where
    ...
    expectedRunLengths ln rs ls =
      case mergePolicyForLevel conf ln ls ul of
        ...
        LevelTiering   -> assertST $ all (\r -> tieringRunSizeToLevel conf r `elem` [ln, ln+1]) rs
          ...
          case (ir, mrs) of
            ...
            (Single r, m) -> do
              assertST $ case m of CompletedMerge{} -> True
                                   OngoingMerge{}   -> False
              assertST $ levellingRunSizeToLevel conf r == ln
        ...
        LevelTiering ->
          case (ir, mrs, mergeTypeForLevel ls ul) of
            ...
            (Single r, m, _) -> do
              assertST $ case m of CompletedMerge{} -> True
                                   OngoingMerge{}   -> False
              assertST $ tieringRunSizeToLevel conf r == ln
            ...
            (_, CompletedMerge r, MergeLastLevel) -> do
              assertST $ ln == 1
              assertST $ tieringRunSizeToLevel conf r <= ln+1
...
newWith conf
  ...
  | sizeRatio conf <= 1 = error "newWith: sizeRatio should be larger than 1"
  | otherwise
  = do
    c   <- newSTRef 0
...
duplicate (LSMHandle _scr conf lsmr) = do
    ...
    return (LSMHandle scr' conf lsmr')
...
increment tr sc conf run0 ls0 ul = do
  ...
  where
    ...
    go !ln incoming [] = do
        let mergePolicy = mergePolicyForLevel conf ln [] ul
    ...
    go !ln incoming (Level ir rs : ls) = do
      ...
      case mergePolicyForLevel conf ln ls ul of
        ...
        LevelTiering | tieringRunSizeToLevel conf r < ln -> do
        ...
        LevelTiering | tieringLevelIsFull conf ln incoming resident -> do
        ...
        -- empty) level .
        LevelLevelling | levellingLevelIsFull conf ln incoming r -> do
...
newLevelMerge tr conf@LSMConfig{..} level mpl mergeType rs = do
  ...
  where
    ...
    nominalDebt = NominalDebt (tieringRunSize conf level)
    ...
    maxPhysicalDebt =
      case mpl of
        LevelLevelling -> sizeRatio * tieringRunSize conf (level-1)
