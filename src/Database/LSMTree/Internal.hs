...
openSession tr hfs hbio dir =
    -- We can not use modifyWithActionRegistry here, since there is no state to
    -- modify. We use withActionRegistry instead, which may have a tiny chance
    -- of leaking resources if openSession is not called in a masked state.
