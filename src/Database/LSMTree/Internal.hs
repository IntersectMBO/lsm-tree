...
flushWriteBuffer tr conf@TableConfig{confDiskCachePolicy}
  ...
  | otherwise = do
    ...
    traceWith tr $ TraceFlushWriteBuffer l size path cache alloc
    r <- allocateTemp reg
...
addRunToLevels tr conf@TableConfig{..} resolve hfs hbio root uniqC r0 reg levels = do
  ...
  where
    ...
    go !ln rs' (V.uncons -> Just (Level mr rs, ls)) = do
        ...
        case mergePolicyForLevel confMergePolicy ln ls of
          ...
          LevelTiering -> do
            ...
            traceWith tr $ TraceAddRun ln (Run.runRunFsPaths r)
    ...
    expectCompletedMerge :: LevelNo -> MergingRun (Handle h) -> m (Run (Handle h))
    expectCompletedMerge ln (SingleRun r)                   = do
      traceWith tr $ TraceCompletedMergeSingleRun ln (Run.runNumEntries r) (Run.runRunFsPaths r)
      pure r
    expectCompletedMerge ln (MergingRun (CompletedMerge r)) = do
      traceWith tr $ TraceCompletedMerge ln (Run.runNumEntries r) (Run.runRunFsPaths r)
      pure r
    ...
    newMerge mergepolicy mergelast ln rs
      ...
      , V.null rest = do
          traceWith tr $ TraceNewMergeSingleRun ln (Run.runNumEntries r) (Run.runRunFsPaths r)
          pure (SingleRun r)
      | otherwise = do
        ...
        traceWith tr $ TraceNewMerge ln (V.map Run.runNumEntries rs) runPaths caching alloc mergepolicy mergelast
        r <- allocateTemp reg
...
{-# SPECIALISE mergeRuns :: ResolveSerialisedValue -> HasFS IO h -> HasBlockIO IO h -> RunDataCaching -> RunBloomFilterAlloc -> RunFsPaths -> Merge.Level -> V.Vector (Run (Handle h)) -> IO (Run (Handle h)) #-}
mergeRuns ::
     m ~ IO
  => ResolveSerialisedValue
  -> HasFS m h
