...
data LSMTreeTrace =
  ...
  | TraceCloseSession
    -- Table
  | TraceNewTable
  | TraceOpenSnapshot SnapshotName (Maybe DiskCachePolicy)
...
openSnapshot ::
  ...
  -> ResolveSerialisedValue
  -> m (Table m h)
openSnapshot sesh mpolicy label tableType snap resolve =
  wrapFileCorruptedErrorAsSnapshotCorruptedError snap $ do
    traceWith (sessionTracer sesh) $ TraceOpenSnapshot snap mpolicy
    withOpenSession sesh $ \seshEnv -> do
      withActionRegistry $ \reg -> do
        ...
        let SnapshotMetaData label' tableType' conf snapWriteBuffer snapLevels mTreeOpt
              = maybe id overrideDiskCachePolicy mpolicy $ snapMetaData
