...
unions ts = do
    sesh <-
      matchSessions ts >>= \case
        Left (i, root, j, root') -> throwIO $ ErrTableUnionSessionMismatch i (getSessionRoot root) j (getSessionRoot root')
...
matchSessions ::
     (MonadSTM m, MonadThrow m)
  => NonEmpty (Table m h)
  -> m (Either (Int, SessionRoot, Int, SessionRoot) (Session m h))
matchSessions = \(t :| ts) ->
    withSessionRoot t $ \root -> do
      eith <- go root 1 ts
      pure $ case eith of
        Left (i, root') -> Left (0, root, i, root')
