...
data Cursor m k v b
  = forall h.
    (Typeable h) =>
    Cursor {-# UNPACK #-} !(Unsafe.Cursor m h)
...
The user is encouraged to implement both functions if this leads to a more efficient implementation.
...
[Valid Output]:
    ...
    prop> (v1 `resolve` v2) `deepseq` True
-}
...
resolveValidOutput v1 v2 =
  (v1 `resolve` v2) `deepseq` True
...
type Session' :: (Type -> Type) -> Type
data Session' m = forall h. Typeable h => Session' !(Unsafe.Session m h)

instance NFData (Session' m) where
  rnf (Session' s) = rnf s

type Table' :: (Type -> Type) -> Type -> Type -> Type -> Type
data Table' m k v b = forall h. Typeable h => Table' (Unsafe.Table m h)

instance NFData (Table' m k v b) where
  rnf (Table' t) = rnf t

type Cursor' :: (Type -> Type) -> Type -> Type -> Type -> Type
data Cursor' m k v b = forall h. Typeable h => Cursor' (Unsafe.Cursor m h)
