...
{- $compact
  ...
  >
  > search1 :: k -> Index1 k -> Maybe pageNr
  > search1 = -- elided
...
data CompactIndex = CompactIndex {
  ...
  , ciTieBreaker           :: !(Map SerialisedKey PageNr)
...
-- | Padded to 64 bit.
--
-- Assumes that the bitvector is byte-aligned.
...
putBitVec (BitVec offsetBits lenBits ba)
  ...
  where
    ...
    totalBytesWritten = divUp8 lenBits

    bitmaskLastByte = unsafeShiftR 0xFF (8 - remainingBits)
    remainingBits = mod8 lenBits

-- | Padded to 64 bit.
putTieBreaker :: Map SerialisedKey PageNr -> BB.Builder
putTieBreaker m =
       BB.word64LE (fromIntegral (Map.size m))
    <> foldMap putEntry (Map.assocs m)
  where
    putEntry :: (SerialisedKey, PageNr) -> BB.Builder
    putEntry (k, PageNr pageNr) =
           BB.word32LE (fromIntegral pageNr)
...
getBitVec ba offset64 numEntries = (offset64', vec)
  where
    ...
    offset64' = offset64 + divUp64 numEntries

getTieBreaker :: ByteArray -> Offset64 -> Map SerialisedKey PageNr
getTieBreaker ba = \offset64 ->
    let size = fromIntegral (indexByteArray ba offset64 :: Word64)
    in Map.fromList $ take size $ unfoldr (Just . getEntry) (offset64 + 1)
  where
    getEntry :: Offset64 -> ((SerialisedKey, PageNr), Offset64)
    getEntry offset64 =
        let offset32 = mul2 offset64
            pageNr = fromIntegral (indexByteArray ba offset32 :: Word32)
            ...
            -- It will stay alive as long as the compact index anyway, so we
            -- probably don't need the 'VP.force', but we expect very few keys
            -- in the tie breaker, so it is cheap to do and we don't have to
            -- worry about the issue anymore.
            key = SerialisedKey' (VP.force (VP.Vector keyOffset8 keyLen8 ba))
        in ((key, PageNr pageNr), offset64 + 1 + divUp8 keyLen8)

mul2, divUp2 :: Int -> Int
mul2 n = unsafeShiftL n 1
divUp2 n = unsafeShiftR (n + 1) 1

mul8, mod8, div8, divUp8 :: Int -> Int
mul8 n = unsafeShiftL n 3
mod8 n = n .&. 7
div8 n = unsafeShiftR n 3
divUp8 n = div8 (n + 7)

mul64, divUp64 :: Int -> Int
mul64 n = unsafeShiftL n 6
divUp64 n = unsafeShiftR (n + 63) 6
