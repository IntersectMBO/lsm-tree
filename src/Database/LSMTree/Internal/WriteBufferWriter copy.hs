...
writeWriteBuffer ::
     (MonadSTM m, MonadST m, MonadCatch m)
  => HasFS m h
  -> HasBlockIO m h
  -> WriteBufferFsPaths
  -> WriteBuffer
  ...
  -> m ()
writeWriteBuffer hfs hbio fsPaths buffer blobs =
    bracketOnError (new hfs hbio fsPaths) remove $ \writer -> do
...
new ::
     (MonadST m, MonadSTM m, MonadCatch m)
  => HasFS m h
  -> HasBlockIO m h
  -> WriteBufferFsPaths
...
new hfs hbio fsPaths = do
    ...
    bracketOnError openKOps removeKOps $ \writerKOpsHandle ->
      bracketOnError openBlob removeBlob $ \writerBlobHandle -> do
  ...
  where
    openKOps = ForKOps <$> makeHandle hfs (writeBufferKOpsPath fsPaths)
    openBlob = ForBlob <$> makeHandle hfs (writeBufferBlobPath fsPaths)

    removeKOps = removeHandle hfs . unForKOps
    removeBlob = removeHandle hfs . unForBlob
...
{-# SPECIALISE dropCaches :: WriteBufferWriter IO h -> IO () #-}
dropCaches :: (MonadSTM m, PrimMonad m, MonadThrow m) => WriteBufferWriter m h -> m ()
dropCaches WriteBufferWriter {..} =
    dropCache writerHasBlockIO (unForKOps writerKOpsHandle)
      `finally`
        dropCache writerHasBlockIO (unForBlob writerBlobHandle)

{-# SPECIALISE close :: WriteBufferWriter IO h -> IO () #-}
-- | Close a 'WriteBufferWriter' without removing its files
close :: (MonadSTM m, PrimMonad m, MonadThrow m) => WriteBufferWriter m h -> m ()
close WriteBufferWriter {..} =
    closeHandle writerHasFS (unForKOps writerKOpsHandle)
      `finally`
        closeHandle writerHasFS (unForBlob writerBlobHandle)

{-# SPECIALISE remove :: WriteBufferWriter IO h -> IO () #-}
-- | Close a 'WriteBufferWriter' and remove its files
remove :: (MonadSTM m, PrimMonad m, MonadThrow m) => WriteBufferWriter m h -> m ()
remove WriteBufferWriter {..} =
    removeHandle writerHasFS (unForKOps writerKOpsHandle)
      `finally`
        removeHandle writerHasFS (unForBlob writerBlobHandle)
