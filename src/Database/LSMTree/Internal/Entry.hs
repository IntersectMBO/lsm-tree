...
-- | Given a value-merge function @f@, combine entries as if composing functions
-- according to these equivalences:
--
-- @@@
--   Delete             ~ Map.delete k
--   Insert v           ~ Map.insert k (v, Semigroup.First Nothing)
--   InsertWithBlob v b ~ Map.insert k (v, Semigroup.First (Just b))
--   Mupdate v          ~ Map.insertWith
--                          (\(u, b) (v, _) -> (f u v, b))
--                          k
--                          (v, Semigroup.First Nothing)
-- @@@
--
-- In particular, a mupdate overwrites any existing blob.
...
-- both have a value, these values get combined monoidally. Only take blobs from
-- the first entry.
