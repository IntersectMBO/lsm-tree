...
updatesWithInterleavedFlushes tr conf resolve hfs hbio root uc es reg tc = do
    ...
    supplyCredits conf (UnscaledCredits numAdded) (tableLevels tc)
...
newtype UnscaledCredits = UnscaledCredits Int
...
scaleCreditsForMerge :: Ref (MergingRun m h) -> UnscaledCredits -> MR.Credits
-- A single run is a trivially completed merge, so it requires no credits.
scaleCreditsForMerge (DeRef mr) (UnscaledCredits c) =
