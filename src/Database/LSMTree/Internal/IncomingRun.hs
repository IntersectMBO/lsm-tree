...
supplyCreditsIncomingRun conf ln (Merging _ nominalDebt nominalCreditsVar mr)
    ...
    if nominalCredits' == nominalCredits
      then
        return deposit
      else do
        let !leftover      = let NominalCredits c   = nominalCredits
                                 NominalCredits c'  = nominalCredits'
                                 NominalCredits dep = deposit
                                 used               = c' - c
                                 left               = dep - used
                             in assert (left >= 0) $ NominalCredits left
            !mergeDebt     = MR.totalMergeDebt mr
