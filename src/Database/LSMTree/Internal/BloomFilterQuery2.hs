...
-- sparse, with roughly M*FPR positive results. So we use a sparse
...
--
-- * Low 64bit word:
--   -    padding and the hash number (MSB)
...
prepInitialCandidateProbes
  ...
  -> RunIxKeyIx
     -- ^ The last candidate added to the rolling buffer. When this hits zero
     -- then no more candidates are added, and we just drain the buffer.
  -> ST s (Int,  RunIxKeyIx)
prepInitialCandidateProbes
    filters keyhashes
    candidateProbes i_w
...
bloomQueriesBody !filters !keyhashes !candidateProbes =
  ...
  where
    ...
    testCandidateProbe, prepNextCandidateProbe
      :: Int -> Int
         -- ^ The read and write indexes within the rolling buffer. This wraps
         -- around. It is represented with the read index in the least significant
         -- 8 bits, and the write index in the next higher 8 bits.
      -> RunIxKeyIx
