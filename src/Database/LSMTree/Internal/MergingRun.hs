...
new mergePolicy mergeNumRuns mergeNumEntries knownCompleted state = do
...
-- FIXME: only use one credit type
...
{-# SPECIALISE supplyCredits :: ScaledCredits -> CreditThreshold -> Ref (MergingRun IO h) -> IO () #-}
...
supplyCredits ::
  ...
  -> Ref (MergingRun m h)
  -> m ()
supplyCredits (ScaledCredits c) creditsThresh (DeRef MergingRun {..}) = do
