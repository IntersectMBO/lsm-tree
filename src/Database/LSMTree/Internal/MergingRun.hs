...
{- |
  Credits are accumulated until they go over the 'CreditThreshold', after which
...
{-# SPECIALISE supplyCredits :: Credits -> CreditThreshold -> Ref (MergingRun IO h) -> IO () #-}
    ...
    if mergeCompleted == MergeKnownCompleted then
      ...
      if totalSteps + unspentCredits' >= unNumEntries mergeNumEntries then do
        ...
        isMergeDone <-
          ...
          -- credits as we took, even if the merge has progressed. See 'Credits'
...
{-# SPECIALISE addUnspentCredits :: UnspentCreditsVar RealWorld -> Credits -> IO Credits #-}
...
{-# SPECIALISE tryTakeUnspentCredits :: UnspentCreditsVar RealWorld -> CreditThreshold -> Credits -> IO (Maybe Credits) #-}
...
{-# SPECIALISE putBackUnspentCredits :: UnspentCreditsVar RealWorld -> Credits -> IO () #-}
...
{-# SPECIALISE takeAllUnspentCredits :: UnspentCreditsVar RealWorld -> IO Credits #-}
...
{-# SPECIALISE stepMerge :: StrictMVar IO (MergingRunState IO h) -> TotalStepsVar RealWorld -> Credits -> IO Bool #-}
stepMerge ::
     (MonadMVar m, MonadMask m, MonadSTM m, MonadST m)
