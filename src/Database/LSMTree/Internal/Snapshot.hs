...
-- nominal credits, not the nominal debt or the merge credits. The rational is
-- a bit subtle.
--
...
-- TableConfig), and on level number that the merge is at (which also known
-- from the snapshot structure).
--
-- The merge credits can be recalculated from the combin] of the nominal debt,
