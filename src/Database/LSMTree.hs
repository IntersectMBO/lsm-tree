...
rangeLookup range (Internal.Table' t) =
...
updates ::
  ...
  -> Table m k v b
  -> m ()
updates es (Internal.Table' t) = do
...
inserts ::
  ...
  -> Table m k v b
  -> m ()
inserts = updates . fmap (\(k, v, b) -> (k, Insert v b))
...
{-# SPECIALISE createSnapshot ::
     (Common.Labellable (k, v, b), ResolveValue v)
  => SnapshotName
  -> Table IO k v b
  -> IO () #-}
createSnapshot :: forall m k v b.
     ( IOLike m
     , Common.Labellable (k, v, b)
     , ResolveValue v
     )
  => SnapshotName
  -> Table m k v b
...
createSnapshot snap (Internal.Table' t) =
  ...
  where
    label = Internal.SnapshotLabel $ Common.makeSnapshotLabel (Proxy @(k, v, b))
