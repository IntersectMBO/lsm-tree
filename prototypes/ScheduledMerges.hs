...
--
-- Trees can be nested arbitrarily deeply, since each input to 'union' might
-- already contain an in-progress merging tree, which then becomes shared
...
--
-- Usually, each input to a pending union merge will be a level merge of the
-- entire content of a table.
-- Vice versa, each of these pending level merges will
-- TODO
--
-- Goes via MergingTree (with its STRef) to allow sharing existing unions.
data PendingMerge s = PendingLevelMerge ![IncomingRun s] !(Maybe (MergingTree s))
...
combineUnion new_ old =
    case (value new_, value old) of
      (Nothing, _)                 -> old
      (_, Nothing)                 -> new_
      (Just (v', b'), Just (v, b)) -> Insert (resolveValue v' v) (resolveBlob b' b)
  where
    value = \case
      Insert v b -> Just (v, b)
      Delete     -> Nothing
