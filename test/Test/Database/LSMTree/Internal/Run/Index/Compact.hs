...
prop_searchMinMaxKeysAfterConstruction csize ps = eqMapProp real model
  where
    ...
    modelSearch m = \case
        OnePageOneKey k       -> do
          c <- incrCounter
          pure $ Map.insert k (SinglePage (PageNr c)) m
        OnePageManyKeys k1 k2 -> do
          c <- incrCounter
          pure $ Map.insert k1 (SinglePage (PageNr c)) $ Map.insert k2 (SinglePage (PageNr c)) m
        MultiPageOneKey k n -> do
          let incr = 1 + fromIntegral n
          c <- plusCounter incr
          pure $ if incr == 1 then Map.insert k (SinglePage (PageNr c)) m
                              else Map.insert k (MultiPage (PageNr c) (PageNr $ c + fromIntegral n)) m
