...
prop_newInExistingDir hfs hbio = do
    ...
    bracket
      (try (RunBuilder.new hfs hbio testSessionSalt runParams (RunFsPaths runDir (RunNumber 17)) (NumEntries 0)))
...
prop_newTwice hfs hbio = do
    ...
    bracket
      (RunBuilder.new hfs hbio testSessionSalt runParams (RunFsPaths runDir (RunNumber 17)) (NumEntries 0))
      RunBuilder.close $ \_ ->
        bracket
          (try (RunBuilder.new hfs hbio testSessionSalt runParams (RunFsPaths runDir (RunNumber 17)) (NumEntries 0)))
          (traverse_ RunBuilder.close) $ pure . \case
