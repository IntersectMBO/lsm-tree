...
data Action' m h a where
  ...
  RangeLookup ::
    ...
    -> Act' m h (V.Vector (QueryResult k v (WrapBlobRef h m b)))
  -- Cursor
  NewCursor ::
       C k v b
    => Maybe k
  ...
  CreateSnapshot ::
    ...
    -> R.SnapshotLabel -> R.SnapshotName -> Var m h (WrapTable h m k v b)
    -> Act' m h ()
  OpenSnapshot   ::
       C k v b
    => {-# UNPACK #-} !(PrettyProxy (k, v, b))
...
instance ( Eq (Class.TableConfig h)
  ...
  data instance ModelValue (ModelState m h) a where
    ...
    MBlobRef :: Class.C_ b
             => Model.BlobRef b -> Val m h (WrapBlobRef h m b)
    -- values
    ...
    MQueryResult :: Class.C k v b
                 => QueryResult k v (Val m h (WrapBlobRef h m b))
                 -> Val m h (QueryResult k v (WrapBlobRef h m b))
    MBlob :: (Show b, Typeable b, Eq b)
          => WrapBlob b -> Val m h (WrapBlob b)
  ...
  data instance Observable (ModelState m h) a where
    ...
    OQueryResult :: Class.C k v b
                 => QueryResult k v (Obs m h (WrapBlobRef h m b))
                 -> Obs m h (QueryResult k v (WrapBlobRef h m b))
    OBlob :: (Show b, Typeable b, Eq b)
          => WrapBlob b -> Obs m h (WrapBlob b)
...
arbitraryActionWithVars _ label ctx (ModelState st _stats) =
  ...
  where
    ...
    unionDescendantTableVars, notUnionDescendantTableVars :: [Var m h (WrapTable h m k v b)]
    ...
    blobRefsVars = fmap (mapGVar (OpComp OpLookupResults)) lookupResultVars
      ...
      where
        ...
        queryResultVars  :: [Var m h (V.Vector (QueryResult k v (WrapBlobRef h m b)))]
    ...
    genUnionActions :: [(Int, Gen (Any (LockstepAction (ModelState m h))))]
    genUnionActions
      | null tableVars = []
      | otherwise =
        [ (2,  fmap Some $ (Action <$> genErrors <*>) $
     ...
     ++ [ (8, fmap Some $ (Action <$> genErrors <*>) $
      ...
      where
        ...
        gen2or3TableVars :: Gen (NonEmpty (Var m h (WrapTable h m k v b)))
        gen2or3TableVars = do
