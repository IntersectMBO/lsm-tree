...
propLockstep_ModelIOImpl =
  ...
  where
    ...
    acquire = do
      ...
      logVar <- newTVarIO emptyErrorsLog
...
propLockstep_RealImpl_MockFS_IO ::
  ...
  -> Actions (Lockstep (ModelState R.Table))
  -> QC.Property
propLockstep_RealImpl_MockFS_IO tr b =
    runActionsBracket'
      (Proxy @(ModelState R.Table))
      (acquire_RealImpl_MockFS tr)
      (release_RealImpl_MockFS b)
      (\r (_, session, errsVar, logVar) -> do
            faultsVar <- newMutVar []
            let
              env :: RealEnv R.Table IO
...
propLockstep_RealImpl_MockFS_IOSim tr actions =
  ...
  where
    ...
    prop = do
        ...
        p <- QC.run $ release_RealImpl_MockFS True (fsVar, session, errsVar, logVar)
        pure
...
release_RealImpl_MockFS ::
  ...
  -> m Property
release_RealImpl_MockFS b (fsVar, session, _, _) = do
...
runRealWithInjectedErrors s env merrs k rollback =
  case merrs of
    Nothing -> do
      ...
      catchErr handlers k
    Just errs -> do
      atomically $ writeTVar logVar emptyErrorsLog
      ...
      case eith of
        Left (Model.ErrFsError _) -> do
          ...
          pure $ assert (sumLengths errsLog >= 1) $ eith
        Left _ ->
          pure $ assert (sumLengths errsLog == 0) $ eith
        Right x -> do
          modifyMutVar faultsVar (InjectFaultAccidentalSuccess s :)
          rollback x
          pure $ assert (sumLengths errsLog == 0) $ Left $ Model.ErrFsError ("dummy: " <> s)
  where
