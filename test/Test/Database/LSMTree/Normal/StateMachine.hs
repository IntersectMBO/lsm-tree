...
import qualified Test.Database.LSMTree.ModelIO.Class as Class
...
data Stats = Stats {
    -- === Tags
    -- | Have we executed an action before? This lets our tagging functions see
    -- whether they are currently inspecting the first action in the sequence.
    updated       :: Bool
...
updateStats action result =
    ...
    . updSnapshotted
  where
    update stats = stats { updated = True }
