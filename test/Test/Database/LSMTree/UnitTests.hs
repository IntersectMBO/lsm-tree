...
unit_twoTableTypes =
    ...
    withSession nullTracer hfs hbio (FS.mkFsPath []) $ \sess -> do
      ...
      table1' <- openSnapshot @_ @Key1 @Value1 @Blob1 sess Nothing label1 snap1
      table2' <- openSnapshot @_ @Key2 @Value2 @Blob2 sess Nothing label2 snap2
...
unit_snapshots =
    ...
    withSession nullTracer hfs hbio (FS.mkFsPath []) $ \sess -> do
      ...
      assertException (ErrSnapshotWrongLabel snap1
        ...
        _ <- openSnapshot @_ @Key2 @Value2 @Blob2 sess Nothing label2 snap1
        return ()

      assertException (ErrSnapshotDoesNotExist snap2) $ do
        _ <- openSnapshot @_ @Key1 @Value1 @Blob1 sess Nothing label2 snap2
        return ()
  where
    snap1, snap2 :: SnapshotName
    snap1 = "table1"
