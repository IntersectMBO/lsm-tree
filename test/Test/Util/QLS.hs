...
class TagSequence state where
  monitorSequence :: Actions state -> Property -> Property

getFinalState :: forall state. StateModel state => Actions state -> state
getFinalState actions = underlyingState finalAnnState
  where
    finalAnnState :: Annotated state
    finalAnnState = stateAfter @state actions

renderTabulated :: [(String, [String])] -> Property -> Property
renderTabulated tabs prop =
    foldr (\tab -> QC.tabulate (fst tab) (snd tab)) prop tabs
