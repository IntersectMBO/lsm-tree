...
data Err
  ...
  | ErrAbortActionRegistry (NonEmpty Err)
  | ErrOther !String
  deriving stock (Eq, Show)

isSnapshotCorrupted :: Err -> Bool
isSnapshotCorrupted (ErrDiskFault _)             = True
...
isSnapshotCorrupted (ErrAbortActionRegistry es)  = firstNotOtherSatisfies isSnapshotCorrupted es
...
isDiskFault (ErrAbortActionRegistry es)  = firstNotOtherSatisfies isDiskFault es
isDiskFault _                            = False

isOther :: Err -> Bool
isOther (ErrOther _)                 = True
isOther (ErrCommitActionRegistry es) = all isOther es
isOther (ErrAbortActionRegistry es)  = all isOther es
isOther _                            = False
