...
new ::
  ...
  => TableConfig
  -> m (Table k v b)
new config = newTableWith config IsNotUnion Model.empty
...
newTableWith ::
  ...
  -> Model.Table k v b
  -> m (Table k v b)
newTableWith config isUnion tbl = state $ \Model{..} ->
  let table = Table {
          tableID = nextID
        , config
        , isUnion
...
createSnapshot label name t@Table{..} = do
    ...
    modify (\m -> m {
        snapshots = Map.insert name (Snapshot config label (toSomeTable $ Model.snapshot table) isUnion False) (snapshots m)
      })
...
duplicate t@Table{..} = do
    ...
    newTableWith config isUnion $ Model.duplicate table
...
-- references. Otherwise it can not invalidate them, because 'updates' do not
-- modify union levels.
data IsUnion = IsUnion | IsNotUnion
  deriving stock (Show, Eq)
...
union r th1 th2 = do
  ...
  newTableWith TableConfig IsUnion $ Model.union r t1 t2
...
unions r tables = do
    tables' <- forM tables $ \table -> do
      ...
      pure table'
    newTableWith TableConfig IsUnion $ Model.unions r tables'
...
-- invalidation conseratively in a similar way to 'updates': any supply of @>=1@
-- union credits is enough to invalidate union blob references.
--
...
-- references for other tables if they share mergin runs in their union levels.
