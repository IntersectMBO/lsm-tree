...
-- | The size of batches that are sent to @HasBlockIO@ by the lookup code.
--
-- This is intentionally the same as 'FS.ioctxBatchSizeLimit'.
benchmarkLookupBatchSize :: BatchSize
benchmarkLookupBatchSize = BatchSize 64

-- | The size of batches as they are generated by the benchmark.
--
-- The lookup code will subdivide batches of size 'benchmarkGenBatchSize' into
-- batches of size 'benchmarkLookupBatchSize' when performing @IO@ using
-- @HasBlockIO@.
...
benchmarks = do
    ...
    _bprepLookups <-
      benchmark "benchPrepLookups"
                ...
                bgenKeyBatches
    _blookupsInBatches <-
      benchmark "benchLookupsInBatches"
                ...
                (benchLookupsInBatches hbio benchmarkLookupBatchSize benchmarkResolveSerialisedValue runs blooms indexes handles keyRng0) benchmarkNumLookups
                bgenKeyBatches
...
benchLookupsInBatches !hbio !batchn !resolve !rs !bs !ics !hs !keyRng !n
  ...
  | otherwise = do
      let (!ks, !keyRng') = genLookupBatch keyRng benchmarkGenBatchSize
      !_ <- lookupsInBatches hbio batchn resolve rs bs ics hs ks
      benchLookupsInBatches hbio batchn resolve rs bs ics hs keyRng' (n-benchmarkGenBatchSize)
