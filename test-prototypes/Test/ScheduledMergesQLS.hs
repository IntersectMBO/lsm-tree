...
instance InLockstep Model where
  ...
  arbitraryWithVars ctx model =
    case findLsmVars ctx of
      []   ->
        ...
        fmap Some $ ANew <$> (LSMConfig <$> choose (1,5) <*> choose (2,5) <*> elements [LazyLevelling, Levelling])
      vars ->
  ...
  shrinkWithVars _ctx _model (ANew conf) =
         [ Some $ ANew conf { maxWriteBufferSize = mwbs' }
         | mwbs' <- shrink mwbs
         , mwbs' >= 1, mwbs' <= 5
         ]
      ++ [ Some $ ANew conf { sizeRatio = sr' }
         | sr' <- shrink sr
         , sr' >= 2, sr' <= 5
         ]
      ++ [ Some $ ANew conf { mergePolicy = mp' }
...
runActionIO action lookUp =
  stToIO $
  case action of
    ANew conf           -> newWith conf >>= \table -> return (table, Dep table)
    AInsert var evk v b -> insert tr table k v b >> return (k, Dep table)
      where
        table = lookupDepTable var
        k = either lookUpVar id evk
    ADelete var evk     -> delete tr table k >> return (Dep table)
      where
        table = lookupDepTable var
        k = either lookUpVar id evk
